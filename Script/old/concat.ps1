# 设置 ffmpeg 可执行文件路径
$ffmpegPath = "C:\Users\Administrator\scoop\apps\ffmpeg\5.1.2\bin\ffmpeg.exe"

# 获取当前文件夹路径
$currentFolder = Get-Location

# 获取当前文件夹中相同日期的视频文件列表，并按时间升序排列
$videoFilesByDate = Get-ChildItem -Path $currentFolder -Filter "录制-*.flv" | Group-Object { $_.LastWriteTime.ToShortDateString() }
